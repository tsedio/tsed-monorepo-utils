...
- Plugin interne:
  - Récupère `nextRelease.version` depuis le contexte semantic-release.
  - Exécute `monoRepo.newVersion({version})` pour propager la version dans le monorepo.
  - Construit les packages: `monoRepo.build("workspace")`.
  - Rafraîchit les installations: `monoRepo.manager.refreshInstall()`.
  - Commit les changements: `monoRepo.commitChanges({version})`.
...
- Plugin interne: `monoRepo.publish("packages", {dryRun})`
  - Publie les packages du monorepo selon la configuration interne (yarn/npm selon détecté), en respectant le `dryRun` si activé.
